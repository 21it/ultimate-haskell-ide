...
if [ -z "$*" ]; then
  echo "ultimate-haskell-ide ==> using defaults"
else
  for ARG in "$@"; do
    case $ARG in
      mini)
      ...
      maxi)
        ...
        ;;
      *)
        echo "ultimate-haskell-ide ==> unrecognized ARG $ARG"
        exit 1
        ;;
    esac
  done
...
echo "ultimate-haskell-ide ==> installing with MINI=$MINI"
...
echo "ultimate-haskell-ide ==> successfully installed with MINI=$MINI"
